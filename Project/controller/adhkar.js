...
function setAdhkarCardsSection() {
  var adhkarNames = document.querySelector(".adhkar-names");
  for (let name in adhkar) {
    let card = createAdhkarCard(name);
    adhkarNames.appendChild(card);
  }
}

setAdhkarCardsSection();

function createAdhkarCard(name) {
  let card = document.createElement("div");
  card.classList.add("card");
  card.innerHTML = `
      <h3 class="name">${name}</h3>
      `;
  addDhkrCardEventListener(card);
  return card;
}

function addDhkrCardEventListener(card) {
  card.addEventListener("click", function () {
    let cardAdhkarName = card.querySelector(".name").textContent;
    let allAdhkar = adhkar[cardAdhkarName];
    removeLastAdhkar();
    setAdhkar(allAdhkar);
  });
}
